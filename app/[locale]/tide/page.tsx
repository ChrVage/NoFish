...
export default async function TidePage({ params, searchParams }: PageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'tide' });
            ...
            {locationData && (
              <>
                <h2 className="text-2xl font-bold text-maritime-teal-800 mb-1">
                  {t('forecastFor')} {locationData.name}
            ...
            {tidePageData?.stationName && (() => {
              ...
              return (
                <p className="text-xs text-gray-400 mt-1">
                  {t('station')} {tidePageData.stationName}
                  {hasCoords && (
                    <> · {t('stationDistance', { distance: formatDistance(haversineDistance(lat, lng, tidePageData.stationLat ?? 0, tidePageData.stationLng ?? 0)) })}</>
                  )}
                  {' '}· {t('chartDatum')}
                </p>
              );
            ...
            <table className="text-sm" style={{ borderSpacing: '1rem 0', borderCollapse: 'separate' }}>
              ...
              <thead>
                <tr className="border-b border-gray-200 text-left text-xs text-gray-400">
                  <th className="pb-2">{t('timeHeader')}</th>
                  <th className="pb-2 text-right">{t('levelHeader')}</th>
                  <th className="pb-2 text-left">{t('typeHeader')}</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {(() => {
                  ...
                  const nextEvent = visibleEvents[0];
                      const trend = nextEvent ? (nextEvent.flag === 'high' ? t('rising') : t('falling')) : null;
                  ...
                  return (
                    <tr className="text-gray-700 bg-gray-50">
                      ...
                      <td className="py-2 text-xs text-gray-400">{t('lastObservation')}{trend && ` – ${trend}`}</td>
                      <td />
                    </tr>
                  );
                })()}
                    ...
                    >
                      ...
                      <td className="py-2 text-center">
                        <FeedbackButton item={{
                          ...
                          summary: `${event.flag === 'high' ? t('high') : t('low')} tide, ${pred.toFixed(1)} cm`,
                        }} />
                      </td>
                    </tr>
                  );
