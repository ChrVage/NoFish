    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #f0faf9 0%, #e6f4f1 100%)' }}>
      <Header>
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <BackButton />
          <h1 className="text-lg font-bold text-gray-800 tracking-tight">{t('title')}</h1>
      ...
      <main className="max-w-3xl mx-auto px-4 py-10 space-y-6">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          {[
            { value: stats.total, label: t('totalLookups'), accent: false },
            { value: stats.last7, label: t('last7Days'), accent: true },
            { value: stats.todayCount, label: t('today'), accent: false },
          ].map((card) => (
            <div
              key={card.label}
            ...
            >
              ...
              {card.label === t('last7Days') && weekTrend !== null && (
                <p
        ...
        {stats.weekly.length > 0 && (
          ...
          >
            <h2 className="text-sm font-bold text-gray-700 tracking-tight mb-5">{t('weeklyActivity')}</h2>
