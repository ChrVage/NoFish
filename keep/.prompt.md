...
## First prio changes
- 100% safety cannot be obtained on the exposed ocean. Max safety score must be lower to indicate that. The rest of the safety must be obtained through other measures than API data. 
- Set a text on the map when anyone first open the app. "Click on the map to get fishing forecasts" 
- The app auto opens the details page when the user has granted access to location, and when it is open for the first time this session. It stops when interacting with the map. Also stop auto-jump to details page when clicking on the menu.
